...
@ignore @web
	Scenario: 07 Test error message when language is not entered
...
@ignore @web
 Scenario: 08 Test error message when the same language is entered
...
@ignore @web
 Scenario: 09 Test error message when the same language with different level is selected
...
@ignore @web
 Scenario: 10 Test AddNew action button is hidden when the seller add fourth language record sucessfully
...
@ignore @web
 Scenario: 11 Test error message when the same language and same level is entered
...
@ignore @web
 Scenario: 12 Test listing languages
...
@ignore @web
 Scenario: 13 Test error message when language and level is not entered
		Given seller do not  enter value in language text box
		And   do not select level from dropdown box
		When  seller clicks on 'Add' action button 
		Then  Alert message, "Please enter language and level" is displayed on top right of the application. Language shouldnot be added.
