...
@mytag
Scenario: Add Language from excel sheet
...
@mytag
 Scenario: Test Add Language sucessfully
...
@mytag
	Scenario: Test error message when language is not entered
...
@mytag
 Scenario: Test error message when the same language is entered
...
@mytag
 Scenario: Test error message when the same language with different level is selected
...
@mytag
 Scenario: Test AddNew action button is hidden when the seller add fourth language record sucessfully
...
@mytag
 Scenario: Test error message when the same language and same level is entered
		...
		@mytag
 Scenario: Test listing languages
...
@mytag
 Scenario: Test error message when language and level is not entered
		Given seller do not  enter value in language text box
		And   do not select level from dropdown box
		When  seller clicks on 'Add' action button 
		Then  Alert message, "Please enter language and level" is displayed on top right of the application. Language shouldnot be added.
